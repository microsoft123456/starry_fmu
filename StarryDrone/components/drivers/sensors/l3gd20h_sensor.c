...
{	
	...
	res |= l3g_write_checked_reg(ADDR_CTRL_REG1,
                          REG1_POWER_NORMAL | REG1_Z_ENABLE | REG1_Y_ENABLE | REG1_X_ENABLE | REG1_ODR_800_CUTOFF_100);
...
{
	rt_err_t res = RT_EOK;
	if(pos == 0)	/* read raw gyr data */
	{
		for(uint16_t i = 0 ; i<size ; i++)
		{
			res = l3gd20h_gyr_read_raw(((int16_t*)buffer)+i*3);
			if(res != RT_EOK)
			{
				return 0;
			}
		}
	}else if(pos == 1)	/* read rad gyr data */
	{
		for(uint16_t i = 0 ; i<size ; i++)
		{
			res = l3gd20h_gyr_read_dps(((float*)buffer)+i*3);
			if(res != RT_EOK)
			{
				return 0;
			}
...
{	
    ...
    res |= rt_device_register(&gyr_device , GYR_DEVICE_NAME, RT_DEVICE_FLAG_RDWR);
