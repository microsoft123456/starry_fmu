...
rt_err_t write_reg(rt_uint8_t reg , rt_uint8_t val)
{
...
rt_err_t read_reg(rt_uint8_t reg , rt_uint8_t* buff)
{
...
rt_err_t write_checked_reg(rt_uint8_t reg , rt_uint8_t val)
{
	rt_uint8_t r_buff;
	rt_err_t res = RT_EOK;
...
{	
	...
	/* enable mag */
	res |= write_checked_reg(ADDR_CTRL_REG2, REG2_AA_FILTER_BW_362HZ_A | REG2_FULL_SCALE_8G_A);
	...
	res |= write_checked_reg(ADDR_CTRL_REG5, REG5_RES_HIGH_M | REG5_ENABLE_T | REG5_RATE_100HZ_M);
	res |= write_checked_reg(ADDR_CTRL_REG6, REG6_FULL_SCALE_2GA_M);
...
{
	rt_err_t res = RT_EOK;
	if(pos == 0)	/* read raw acc data */
	{
		for(uint16_t i = 0 ; i<size ; i++)
		{
			res = lsm303d_acc_read_raw(((int16_t*)buffer)+i*3);
			if(res != RT_EOK)
			{
				return 0;
			}
	...
	else if(pos == 1)	/* read raw mag data */
	{
		for(uint16_t i = 0 ; i<size ; i++)
		{
			res = lsm303d_mag_read_raw(((int16_t*)buffer)+i*3);
			if(res != RT_EOK)
			{
				return 0;
			}
	...
	else if(pos == 2)	/* read acc data */
	{
		for(uint16_t i = 0 ; i<size ; i++)
		{
			res = lsm303d_acc_measure(((float*)buffer)+i*3);
			if(res != RT_EOK)
			{
				return 0;
			}
	...
	else if(pos == 3)	/* read mag data */
	{
		for(uint16_t i = 0 ; i<size ; i++)
		{
			res = lsm303d_mag_measure(((float*)buffer)+i*3);
			if(res != RT_EOK)
			{
				return 0;
			}
...
{	
    ...
    res |= rt_device_register(&acc_mag_device , ACC_MAG_DEVICE_NAME, RT_DEVICE_FLAG_RDWR);
